import { useEffect, useMemo } from "react";
import { MapContainer, TileLayer, Circle, Popup, Tooltip, useMap } from "react-leaflet";

const KNOWN_HEAT = {
  mumbai: 42, delhi: 47, singapore: 35, tokyo: 38, phoenix: 48,
  amsterdam: 30, london: 29, "new york": 36, dubai: 49, beijing: 40,
  "los angeles": 39, seoul: 37, pune: 43, chennai: 44, kolkata: 41,
  hyderabad: 42, bangalore: 38, ahmedabad: 46, jaipur: 47, lucknow: 44,
  cairo: 44, jakarta: 43, bangkok: 45, karachi: 46, tehran: 43,
  "sao paulo": 33, lagos: 41, chicago: 32,
};

function makeRng(seed) {
  let s = seed | 0;
  return () => { s = Math.imul(s, 1664525) + 1013904223 | 0; return (s >>> 0) / 4294967296; };
}
function strHash(str) {
  let h = 5381;
  for (let i = 0; i < str.length; i++) h = (Math.imul(h, 33) ^ str.charCodeAt(i)) | 0;
  return Math.abs(h);
}
function estimateHeatIndex(lat) {
  const a = Math.abs(lat);
  if (a < 15) return 44; if (a < 25) return 41; if (a < 35) return 38;
  if (a < 45) return 34; if (a < 55) return 30; return 26;
}

// Temperature ranges per zone type [min, max]
const ZONE_TEMP_RANGE = [
  [22, 29],   // Cool
  [30, 37],   // Warm
  [38, 44],   // Hot
  [45, 52],   // Critical
];

function generateZones(lat, lon, cityName, baseHeatIndex) {
  const rng    = makeRng(strHash(cityName.toLowerCase()));
  const zones  = [];
  const spread = 0.08;

  let weights;
  if (baseHeatIndex >= 46)      weights = [0, 5,  35, 60];
  else if (baseHeatIndex >= 42) weights = [0, 10, 50, 40];
  else if (baseHeatIndex >= 38) weights = [5, 25, 55, 15];
  else if (baseHeatIndex >= 34) weights = [15, 50, 30, 5];
  else if (baseHeatIndex >= 30) weights = [30, 50, 18, 2];
  else                          weights = [60, 35,  5, 0];

  const total = weights.reduce((a, b) => a + b, 0);
  const cum   = weights.map(((s) => (v) => (s += v, s))(0)).map((v) => v / total);

  for (let i = 0; i < 22; i++) {
    const angle  = rng() * 2 * Math.PI;
    const dist   = Math.sqrt(rng()) * spread;
    const zLat   = lat + dist * Math.cos(angle);
    const zLon   = lon + dist * Math.sin(angle) / Math.cos((lat * Math.PI) / 180);

    const r = rng();
    let zi = r < cum[0] ? 0 : r < cum[1] ? 1 : r < cum[2] ? 2 : 3;
    if (dist < spread * 0.3 && zi < 2) zi = Math.min(zi + 1, 3);

    // Assign a specific temperature within this zone's range
    const [tMin, tMax] = ZONE_TEMP_RANGE[zi];
    const temp = Math.round(tMin + rng() * (tMax - tMin));

    const radius = 800 + rng() * 1800;
    zones.push({ lat: zLat, lon: zLon, zoneIdx: zi, radius, temp });
  }
  return zones;
}

const ZONE_DEFS = [
  { label: "Cool",     heatRange: "< 30°C",  fillColor: "#22c55e", border: "#15803d", bg: "bg-green-500",  tooltipBg: "#14532d" },
  { label: "Warm",     heatRange: "30–38°C", fillColor: "#eab308", border: "#a16207", bg: "bg-yellow-500", tooltipBg: "#713f12" },
  { label: "Hot",      heatRange: "38–45°C", fillColor: "#f97316", border: "#c2410c", bg: "bg-orange-500", tooltipBg: "#7c2d12" },
  { label: "Critical", heatRange: "> 45°C",  fillColor: "#ef4444", border: "#991b1b", bg: "bg-red-500",    tooltipBg: "#7f1d1d" },
];

function FlyTo({ coords }) {
  const map = useMap();
  useEffect(() => {
    if (coords) map.flyTo([coords.lat, coords.lon], 12, { duration: 1.8 });
  }, [coords, map]);
  return null;
}

export default function MapSection({ searchCoords, searchQuery }) {
  const hasSearch = !!searchCoords;

  const zones = useMemo(() => {
    if (!searchCoords) return [];
    const key       = (searchQuery || "").toLowerCase().trim();
    const heatIndex = KNOWN_HEAT[key] ?? estimateHeatIndex(searchCoords.lat);
    return generateZones(searchCoords.lat, searchCoords.lon, key || "city", heatIndex);
  }, [searchCoords, searchQuery]);

  const zoneCounts = useMemo(() => {
    const c = [0, 0, 0, 0];
    zones.forEach((z) => c[z.zoneIdx]++);
    return c;
  }, [zones]);

  const avgTemp = useMemo(() => {
    if (!zones.length) return null;
    return Math.round(zones.reduce((s, z) => s + z.temp, 0) / zones.length);
  }, [zones]);

  return (
    <section id="map" className="p-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-6 gap-4">
        <div>
          <h2 className="text-3xl font-bold">
            Urban <span className="text-orange-400">Heat Zone</span> Map
          </h2>
          {hasSearch ? (
            <div className="flex items-center gap-4 mt-1 flex-wrap">
              <p className="text-orange-300 text-sm flex items-center gap-1">
                <span>📍</span>
                <span className="truncate max-w-sm">{searchCoords.label}</span>
              </p>
              {avgTemp && (
                <span className="bg-orange-900/40 border border-orange-700/30 text-orange-300 text-xs font-semibold px-3 py-1 rounded-full">
                  Avg zone temp: {avgTemp}°C
                </span>
              )}
            </div>
          ) : searchQuery && !searchCoords ? (
            <p className="text-red-400 mt-1 text-sm">⚠️ "{searchQuery}" not found — try a different spelling.</p>
          ) : (
            <p className="text-gray-400 mt-1 text-sm">🔍 Search a city above to see its neighborhood-level heat zones with temperatures.</p>
          )}
        </div>

        <div className="flex flex-wrap gap-4 text-sm">
          {ZONE_DEFS.map(({ label, bg, heatRange }) => (
            <span key={label} className="flex items-center gap-2 text-gray-300">
              <span className={`w-3 h-3 rounded-full ${bg} inline-block`} />
              <span>{label}</span>
              <span className="text-gray-500 text-xs">{heatRange}</span>
            </span>
          ))}
        </div>
      </div>

      {/* Zone summary */}
      {hasSearch && (
        <div className="flex flex-wrap gap-3 mb-4">
          {ZONE_DEFS.map(({ label, bg, fillColor }, i) => (
            <div key={label} className="bg-slate-900 border border-slate-700 rounded-xl px-4 py-2 flex items-center gap-2 text-sm">
              <span className={`w-3 h-3 rounded-full ${bg}`} />
              <span className="text-gray-400">{label}:</span>
              <span className="font-bold text-white">{zoneCounts[i]}</span>
              <span className="text-gray-500">zones</span>
              <span className="text-gray-600 text-xs">
                ({ZONE_TEMP_RANGE[i][0]}–{ZONE_TEMP_RANGE[i][1]}°C)
              </span>
            </div>
          ))}
        </div>
      )}

      <div className="h-[520px] rounded-3xl overflow-hidden border border-orange-900/40 shadow-lg">
        <MapContainer
          center={hasSearch ? [searchCoords.lat, searchCoords.lon] : [20, 0]}
          zoom={hasSearch ? 12 : 2}
          style={{ height: "100%", width: "100%" }}
          scrollWheelZoom
        >
          <TileLayer
            attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> &copy; <a href="https://carto.com">CARTO</a>'
            url="https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png"
          />
          {searchCoords && <FlyTo coords={searchCoords} />}

          {zones.map((z, i) => {
            const def = ZONE_DEFS[z.zoneIdx];
            return (
              <Circle
                key={i}
                center={[z.lat, z.lon]}
                radius={z.radius}
                pathOptions={{
                  color: def.border, fillColor: def.fillColor,
                  fillOpacity: 0.48, weight: 1.5,
                }}
              >
                {/* Always-visible temperature tooltip */}
                <Tooltip
                  permanent
                  direction="center"
                  offset={[0, 0]}
                  opacity={1}
                  className="heat-tooltip"
                >
                  <span style={{
                    background: def.tooltipBg,
                    color: "#fff",
                    fontWeight: 700,
                    fontSize: 11,
                    padding: "2px 6px",
                    borderRadius: 6,
                    border: `1px solid ${def.border}`,
                    whiteSpace: "nowrap",
                    display: "block",
                    lineHeight: 1.4,
                  }}>
                    {z.temp}°C
                  </span>
                </Tooltip>

                {/* Click popup for full detail */}
                <Popup>
                  <div style={{ fontFamily: "sans-serif", minWidth: 170 }}>
                    <p style={{ fontWeight: 700, fontSize: 15, marginBottom: 6 }}>
                      {def.label} Zone
                    </p>
                    <p style={{ marginBottom: 4, fontSize: 13 }}>
                      🌡️ Temperature: <strong>{z.temp}°C</strong>
                    </p>
                    <p style={{ marginBottom: 4, fontSize: 13, color: "#6b7280" }}>
                      Range: {def.heatRange}
                    </p>
                    <p style={{ fontSize: 12, color: "#6b7280" }}>
                      Zone area: ~{(Math.PI * z.radius * z.radius / 1e6).toFixed(2)} km²
                    </p>
                  </div>
                </Popup>
              </Circle>
            );
          })}
        </MapContainer>
      </div>

      {!hasSearch && (
        <div className="mt-4 bg-slate-900 border border-orange-900/30 rounded-2xl px-6 py-4 text-center text-gray-400 text-sm">
          Use the <span className="text-orange-400 font-semibold">search bar above</span> to look up any city —
          the map zooms in and shows neighborhood Cool / Warm / Hot / Critical zones with live temperatures.
        </div>
      )}

      <p className="text-gray-600 text-sm mt-3 text-right">
        Map: © CartoDB · Zones: EcoSutra AI heat model · Click circles for details
      </p>
    </section>
  );
}
